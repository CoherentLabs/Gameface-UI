...
- **`/tests/src/views/components-e2e`**: Contains the main entry point for the e2e tests. It is a view like any other, so it is just the `ComponentsE2e.tsx` component and its styles - the HTML document around it is generated.
...
If a dev server is already running on port `3000`, the Gameface Player may fail to open and run the tests. Close any open dev servers first, manually or with a tool like `npx kill-port 3000`.
:::
...
5. **Add the component to the test view**: To make your component visible in the test environment, add a new tab or section in `/tests/src/views/components-e2e/ComponentsE2e.tsx`. This ensures your component is properly rendered and testable during development.
