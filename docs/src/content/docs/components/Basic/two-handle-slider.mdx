...
Every component ships with the boilerplate. If you are working in your own project, add `TwoHandleSlider` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add TwoHandleSlider
```

The CLI pulls in whatever `TwoHandleSlider` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
