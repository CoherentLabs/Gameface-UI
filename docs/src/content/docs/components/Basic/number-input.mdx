...
Every component ships with the boilerplate. If you are working in your own project, add `NumberInput` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add NumberInput
```

The CLI pulls in whatever `NumberInput` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
| `value`       | `string \| number`                      | `""`   | The value of the input. Updating this prop is reflected in the input (clamped to `min`/`max`), allowing it to be controlled. Also sets the initial value. |
...
To interact with the `NumberInput` programmatically, you can use the `NumberInputRef` interface. This interface provides properties and methods to access and manipulate the component's state.
...
| `value`    | `Accessor<string>`   | Returns the current value of the input reactively. Use `value()`to read the latest input.                                            |
