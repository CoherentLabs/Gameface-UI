...
For example, you can bind the `increaseValue()` and `decreaseValue()` methods to existing or custom actions:
...
const App = () => {
    ...
    return (
        <NumberInput
            ref={inputRef!}
            onAction={{
                'move-up': () => inputRef.increaseValue(),
                'move-down': () => inputRef.decreaseValue()
            }}
        />
    );
};
