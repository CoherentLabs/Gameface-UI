...
:::note
In `multiple` mode, read the current selection with `ref.selectedValues()` and clear it with `ref.deselectAll()`.
:::
...
:::note
When `value` is provided, it is the source of truth for the selection and overrides the `selected` prop on individual `Dropdown.Option` slots.
:::
