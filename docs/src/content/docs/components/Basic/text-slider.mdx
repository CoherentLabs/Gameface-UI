...
Every component ships with the boilerplate. If you are working in your own project, add `TextSlider` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add TextSlider
```

The CLI pulls in whatever `TextSlider` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
| `value`    | `string`                            | `undefined`   | The current value of the slider. Use this to control or set the slider's position. The prop is reactive - updating it externally moves the handle. |
...
const App = () => {
    return (
        <TextSlider value="Medium" values={['Easy', 'Medium', 'Hard']} >
            <TextSlider.Handle style={{ 'background-color': 'red' }} styleActive={{ 'background-color': 'green' }} />
        </TextSlider>
    );
};
