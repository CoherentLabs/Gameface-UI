...
| `delay`    | `number`                   | `undefined` | When set, debounces the `onChange` callback by the given amount in milliseconds — it fires once typing pauses for that duration instead of on every keystroke. |
