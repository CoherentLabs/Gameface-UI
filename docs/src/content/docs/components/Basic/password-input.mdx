...
Every component ships with the boilerplate. If you are working in your own project, add `PasswordInput` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add PasswordInput
```

The CLI pulls in whatever `PasswordInput` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
<CodePreviewGFUI>
```tsx
import PasswordInput from '@components/Basic/Input/PasswordInput/PasswordInput';
...
| `value`       | `string`                               | `""`   | The value of the input. Updating this prop is reflected in the input, allowing it to be controlled. Also sets the initial value. |
...
| `delay`    | `number \| boolean`                   | `undefined` | When set, debounces the `onChange` callback by the given amount in milliseconds — it fires once typing pauses for that duration instead of on every keystroke. If set to `true`, the default delay of `250ms` is used. |
...
To interact with the `PasswordInput` programmatically, you can use the `PasswordInputRef` interface. This interface provides properties and methods to access and manipulate the component's state.
...
| `visible`  | `Accessor<boolean>` | Indicates whether the password is currently visible. Use `visible()` to check. |
