...
:::tip
The default background has absolute position and covers the entire slider area by having width and height set to `100%`. If you want to add a custom background, make sure to set the width and height of the background element to `100%` and the position to `absolute` as it is shown in the usage section of the `XYSlider.Background` slot.
:::
...
const App = () => {
    return (
        <XYSlider>
            <XYSlider.Background>
                <div style={{width: '100%', height: '100%', position: 'absolute', 'background-image': 'linear-gradient(to right bottom, #ffffff, #b9b9b9, #777777, #3b3b3b, #000000)'}}>
                    Custom Background
                </div>
            </XYSlider.Background>
        </XYSlider>
