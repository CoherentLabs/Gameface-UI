...
Every component ships with the boilerplate. If you are working in your own project, add `XYSlider` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add XYSlider
```

The CLI pulls in whatever `XYSlider` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
|------------|---------------------|-----------------------------------------------------------------------------|
| `element`  | `HTMLDivElement`    | A reference to the XYSlider's root HTML element, useful for DOM access or styling. |
| `value`    | `Accessor<{x: number, y: number}>`  | The current value of the slider. Use `value()` to get the latest value.      |
