...
`Keybinds` comes with the boilerplate and cannot be added to an existing project with the CLI. It depends on a generated type file, an icon-generation script, and the gamepad glyph assets that only exist in the full template, so an installed copy would not build.
