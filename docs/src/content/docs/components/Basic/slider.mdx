...
Every component ships with the boilerplate. If you are working in your own project, add `Slider` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add Slider
```

The CLI pulls in whatever `Slider` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
| `value`       | `number`                            | `undefined`   | The current value of the slider. Use this to control or set the slider's position. The prop is reactive - updating it externally moves the handle. |
...
| `onChange` | `(value: number) => void`              | `undefined` | A function that is called whenever the slider's value changes. It only fires when the value actually changes, not on every mouse move. |
| `onChangeEnd` | `(value: number) => void`           | `undefined` | A function that is called when the user finishes interacting with the slider (e.g. on mouse release). Useful for committing the final value without reacting to every intermediate change. |
...
| `stepValue`    | `direction: 1 \| -1` | `void`    | Steps the value up (`1`) or down (`-1`) by the `step` amount. |
...
| `styleActive` | `JSX.CSSProperties` | `{}` | Inline styles applied to the handle while it is active (the slider is focused via keyboard/gamepad navigation). Merged on top of `style`. |
| `classActive` | `string` | `""` | CSS class applied to the handle while it is active, replacing the default active styling. |
...
const App = () => {
    return (
        <>
            <Slider min={0} max={100} value={35} step={1}>
+               <Slider.Handle style={{'background-color': 'red'}} styleActive={{'background-color': 'green'}}></Slider.Handle>
            </Slider>
        </>
    );
};
...
### React only to the final value

- The `onChange` prop fires every time the value changes while dragging. If you only care about the final value once the user finishes interacting (e.g. to avoid expensive work on every step), use the `onChangeEnd` prop instead.

`onChangeEnd` is called when the user releases the handle after a drag, and after navigation actions (`move-left` / `move-right`) settle.
...
const App = () => {
    ...
    return (
        <>
            <Slider onChangeEnd={(value) => setSliderValue(value)} value={50} max={100} min={0} step={1} />
            <div>{sliderValue()}</div>
        </>
    );
};
