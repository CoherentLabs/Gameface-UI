...
You can also choose between a horizontal (default) or vertical orientation using the `vertical` prop.
...
| `orientation` | `string`                            | `horizontal`  | Sets the direction of the slider. Use `"horizontal"` for a standard slider or `"vertical"` for a vertical slider. |
...
Additionally, by enabling the `polsWithoutText` prop, smaller unlabeled pols are rendered between the main pols for finer granularity and enhanced visual feedback.
