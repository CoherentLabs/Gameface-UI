...
| `navigation-actions` | `Record<string, () => void>` | `undefined` | Extends or overrides the component's default navigation action handlers. See [Navigation Actions](#navigation-actions) for details. |
...
You can extend the Stepper with additional navigation actions or override the default behavior using the `navigation-actions` prop:
