...
Every component ships with the boilerplate. If you are working in your own project, add `Stepper` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add Stepper
```

The CLI pulls in whatever `Stepper` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
<Stepper
    onAction={{
        'select': () => console.log('Current option selected!'),
        'back': () => console.log('Going back from stepper')
    }}
>
    <Stepper.Items>
        <Stepper.Item value="red">red</Stepper.Item>
        <Stepper.Item value="green">green</Stepper.Item>
        <Stepper.Item value="blue">blue</Stepper.Item>
    </Stepper.Items>
</Stepper>
