...
Every component ships with the boilerplate. If you are working in your own project, add `Checkbox` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add Checkbox
```

The CLI pulls in whatever `Checkbox` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
To interact with the `Checkbox` programmatically, you can use the `CheckboxRef` interface. This interface provides properties and methods to access and manipulate the component's state.
...
| `element`  | `HTMLDivElement`    | A reference to the checkbox's root HTML element, useful for DOM access or styling. |
