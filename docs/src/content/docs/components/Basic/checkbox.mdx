...
| `ref`      | `CheckboxRef  \| undefined`            | `undefined` | A reference to the component that gives you access to its methods and the underlying HTML element. Useful if you need to toggle the checkbox programmatically.                                                             |
...
|---|---|---|---|
| `toggle` | `(e?: MouseEvent) => void`  | `void` | Toggles the checkbox |
...
:::caution
If the `Label` slot is used other `children` of the `Checkbox` component won't be rendered.  
:::
...
The `Checkbox` component lets you toggle the `checked` state through code.

#### Steps

1. Declare a `ref` variable with type `CheckboxRef` and pass it to the `Checkbox` component.
2. Call `ref.toggle()` from anywhere (e.g., a keyboard event or external button) to toggle the checked state programmatically.
