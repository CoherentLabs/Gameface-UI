...
Every component ships with the boilerplate. If you are working in your own project, add `InlineTextBlock` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add InlineTextBlock
```

The CLI pulls in whatever `InlineTextBlock` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
<>
    <InlineTextBlock>
        Text with <img width="50" height="50" src="https://images.pexels.com/photos/7623316/pexels-photo-7623316.jpeg"/> image.
    </InlineTextBlock>
</>
```
</CodePreviewGFUI>
