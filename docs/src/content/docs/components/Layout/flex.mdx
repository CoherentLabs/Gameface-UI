...
Every component ships with the boilerplate. If you are working in your own project, add `Flex` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add Flex
```

The CLI pulls in whatever `Flex` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
| `gap` | `string` | `undefined` | Applies spacing between the elements. It's a shorthand for both row and column gaps. Requires Gameface 2.2+. |
| `row-gap` | `string` | `undefined` | Vertical spacing between rows of flex items when the container wraps (`flex-wrap: wrap`). Requires Gameface 2.2+. |
| `column-gap` | `string` | `undefined` | Horizontal spacing between columns of flex items in a row (i.e., along the cross axis when `direction="row"`). Requires Gameface 2.2+. |
