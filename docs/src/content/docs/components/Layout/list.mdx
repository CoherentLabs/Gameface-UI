...
Every component ships with the boilerplate. If you are working in your own project, add `List` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add List
```

The CLI pulls in whatever `List` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
use the `bullet-class` prop and define your styles in the view's `global.scss`. The
class has to be global - a `*.module.scss` would hash the name and `bullet-class`
