...
Every component ships with the boilerplate. If you are working in your own project, add `Transform` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add Transform
```

The CLI pulls in whatever `Transform` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
