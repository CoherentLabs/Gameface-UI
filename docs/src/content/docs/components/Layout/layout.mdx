...
Every component ships with the boilerplate. If you are working in your own project, add `Layout` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add Layout
```

The CLI pulls in whatever `Layout` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
## Slots :badge[3.0.0]{variant="note"}
