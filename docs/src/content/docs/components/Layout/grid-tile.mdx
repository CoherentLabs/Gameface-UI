...
Every component ships with the boilerplate. If you are working in your own project, add `GridTile` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add GridTile
```

The CLI pulls in whatever `GridTile` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
