...
Every component ships with the boilerplate. If you are working in your own project, add `Layout3D` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add Layout3D
```

The CLI pulls in whatever `Layout3D` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
<Layout3D distance='1000px'>
    <Transform matrix={{
        rotate: { y: 60 },
        scale: {x: 2, z: 2},
    }}>Element rotated by 60° on the y-axis. 
    </Transform>
</Layout3D>
```
</CodePreviewGFUI>
