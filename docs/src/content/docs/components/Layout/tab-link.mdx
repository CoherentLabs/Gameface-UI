...
Every component ships with the boilerplate. If you are working in your own project, add `TabLink` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add TabLink
```

The CLI pulls in whatever `TabLink` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
