...
Every component ships with the boilerplate. If you are working in your own project, add `Relative` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add Relative
```

The CLI pulls in whatever `Relative` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
If you need to remove content from the document flow entirely (e.g., for tooltips, modals, or overlays), consider using the [Absolute Component](/components/layout/absolute), which positions elements relative to their closest positioned ancestor or the viewport. 
...
The `center` prop of the `Absolute` component easily centers it within its closest relatively positioned ancestor. You can center it on both axes or restrict it to a single axis.
