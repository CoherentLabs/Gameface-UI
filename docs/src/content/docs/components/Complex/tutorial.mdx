...
To use the Tutorial component, wrap your existing UI with it and wrap every elemnt you wish to be part of the tutorial with the `Tutorial.Step` component and provide it with an order.
...
| `title`    | `string`          | inherits | Tooltip title. If omitted, the previous step's title is reused.          |
| `content`  | `string`          | inherits | Tooltip content. If omitted, the previous step's content is reused.      |
| `outset`   | `number`          | global   | Per-step offset. Overrides the `Tutorial`'s `outset` for this step only. |
...
| ---------- | ------------------------------------ | --------------------------------------------------------------------------------------- |
| `title`    | `string \| undefined`                | The current step's title. Falls back to the previous step's title if omitted.           |
| `content`  | `string \| JSX.Element \| undefined` | The tooltip's main content. Falls back to the previous step's content if omitted.       |
...
:::tip
The `outset` also accept negative values, allowing you tho "shrink" the highlight rectangle around the target element.
:::
