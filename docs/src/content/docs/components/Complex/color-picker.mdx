...
Every component ships with the boilerplate. If you are working in your own project, add `ColorPicker` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add ColorPicker
```

The CLI pulls in whatever `ColorPicker` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
| `onAction` | `Record<string, (scope?: string, ...args: any[]) => void>` | `undefined` | Allows you to add custom navigation action handlers to the color picker. See the [Navigation component documentation](/components/utility/navigation#extending-component-navigation-actions) for details. |
