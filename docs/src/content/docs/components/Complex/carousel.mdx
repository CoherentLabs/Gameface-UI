...
|---------------------------|------------------|--------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `next`                   | None             | `void`       | Moves the carousel to the next item. If at the last item, it wraps around to the last item. If `groupItems` is enabled, it moves to the next group of items.                                               |
| `prev`                   | None             | `void`       | Moves the carousel to the previous item. If at the first item, it wraps around to the first item. If `groupItems` is enabled, it moves to the previous group of items.                                      |
...
#### How the `selected` prop works :badge[Important]{variant="caution"}

The `selected` prop on a `Carousel.Item` only determines the initially active item when the carousel first loads. After initialization, the carousel manages its own selection state as you navigate forward or backward.

When adding or removing items dynamically, the carousel will not automatically update its selection. Instead, the behavior depends on where the new item is inserted:

- **Adding before the selected item**

If you insert a new item before the currently selected one, all items will shift to the right, but the carousel will still consider the same position as selected.
For example, if the carousel has `['Item 1', 'Item 2']` and `Item 2` is selected, inserting a new item at the start will produce `['New Item', 'Item 1', 'Item 2']`, and the carousel will now mark `Item 1` as selected (since it occupies the same index as before).

<video src="/video/carousel/add-item-at-start.webm" autoplay loop muted></video>

- **Adding after the selected item**

If the new item is appended after the current selection, nothing changes - the same item remains selected.

<video src="/video/carousel/add-item-at-end.webm" autoplay loop muted></video>

- **Adding item and mark it as selected**

If you want to dynamically insert an item and immediately make it the selected one, you must assign it the `selected` prop before rendering it.

<video src="/video/carousel/add-selected-item.webm" autoplay loop muted></video>

The following example demonstrates all of these scenarios by adding new items through different buttons:
...
const App = () => {
    ...
    return (
        <>
            <Flex direction='row'>
                <button style={btnStyles} onClick={() => setItems((prev) => [{ name: 'First', selected: false }, ...prev])}>Add non selected item at start</button>
                <button style={btnStyles} onClick={() => setItems((prev) => [...prev, { name: 'Last', selected: false }])}>Add non selected item at end</button>
                <button style={btnStyles} onClick={() => setItems((prev) => [{ name: 'Selected', selected: true }, ...prev])}>Add selected item</button>
            </Flex>
                    ...
                    <For each={items()}>
                        {(item) =>
                            <Carousel.Item
                                ...
                                style-selected={{ transform: 'scale(1.1)', border: '0.2vmax solid #868599' }}
                                style={{ color: 'white', overflow: 'hidden', transition: 'transform 300ms', 'border-radius': '1vmax' }}
                            >
                                <Flex align-items='center' justify-content='center' style={{ background: '#3e3d5d', height: '100%' }}>
                                    {item.name}
                                </Flex>
