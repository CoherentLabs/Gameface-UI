...
Every component ships with the boilerplate. If you are working in your own project, add `Tooltip` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add Tooltip
```

The CLI pulls in whatever `Tooltip` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
To interact with the `Tooltip` programmatically, you can use the `TooltipRef` interface. This interface provides properties and methods to access and manipulate the component's state.

### Properties

| Property   | Type             | Description                                                                 |
|------------|------------------|-----------------------------------------------------------------------------|
| `element`  | `HTMLDivElement` | A reference to the Tooltip's wrapper root HTML element, useful for DOM access or styling. |
...
const App = () => {
    ...
    return (
        <Tooltip message="Press back/ESC to dismiss">
            <div tabindex={1}>Focus me and press ESC</div>
        </Tooltip>
    );
};
```
...
### Using the tooltip with an absolutely positioned element