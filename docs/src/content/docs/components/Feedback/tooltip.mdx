...
const App = () => {
    ...
    return (
        <Tooltip style={{ display: 'flex', "justify-content": 'center', color: 'white', width: '10vmax' }} message="Tooltip text">
            Text
        </Tooltip>
    );
};
...
|-------------|---------------------|---------------|----------------------------------------------------------------------------------------------|
| `content`   | `JSX.Element` | `(props: { message: string }) => JSX.Element`| The content of the tootip message. Can be a JSX element. If not specified, the `message` prop will be used to display a simple text message. |
...
By default, if the `content` option is not provided, the `createTooltip` function returns a component that simply displays the text passed via the `message` prop.
