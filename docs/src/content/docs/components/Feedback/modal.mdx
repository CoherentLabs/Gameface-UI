...
Every component ships with the boilerplate. If you are working in your own project, add `Modal` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add Modal
```

The CLI pulls in whatever `Modal` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
To interact with the `Modal` programmatically, you can use the `ModalRef` interface. This interface provides properties and methods to access and manipulate the component's state.

### Properties

| Property   | Type             | Description                                                                 |
|------------|------------------|-----------------------------------------------------------------------------|
| `element`  | `HTMLDivElement` | A reference to the modal's root HTML element, useful for DOM access or styling. |
