...
Every component ships with the boilerplate. If you are working in your own project, add `Navigation` with the [Gameface CLI](https://frontend-tools.coherent-labs.com/gameface-cli/getting-started/):

```bash
npx gameface-cli add Navigation
```

The CLI pulls in whatever `Navigation` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.
...
:::tip
For detailed usage instructions, examples, and advanced features, refer to the [Guide](#guide) section.
:::
...
const App = () => {
    const defaultActions: ActionMap = {
        ...
        'select': {key: {binds: ['SPACEBAR'], type: ['press']}, button: {binds: ['face-button-left'], type: 'press'}},
...
- **Global emission** - They emit events via the [event bus](/docs/concepts/ui-communication/), allowing any component to respond
...
const customActions: ActionMap = {
    // Customize the 'select' action to use SPACEBAR instead of ENTER
    'select': {
...
| `global`   | `boolean`                                    | When `true`, the action emits globally via the [eventBus](/docs/concepts/ui-communication/), allowing any component to listen. When `false`, only the callback is executed. Default actions have this set to `true`.                          |
...
| `ref`             | `NavigationRef` \| `((nav: NavigationRef) => void)` | `undefined` | A reference to the component, providing access to all navigation methods via the [NavigationRef](#usenavigation-hook--ref-api) interface.                                                                                                   |
...
|-----------------|------------------------------|--------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `addAction`     | `name: ActionName, config:` [ActionCfg](#action-configuration-reference-actioncfg) | `void` | Adds a new `Navigation` action and registers it with the interaction manager. |
| `removeAction`  | `name: ActionName`           | `void`       | Removes a registered `Navigation` action and unregisters it from the interaction manager.                                                                                                                     |
| `updateAction`  | `name: ActionName, config:` [ActionCfg](#action-configuration-reference-actioncfg) | `void` | Updates an existing action's configuration. Default actions can be updated as well. Accepts the same config properties as `addAction`.                                                                      |
