...
:::note
Assign nested slots to the parent component using `Object.assign` to ensure they are easily accessible. For example, assigning `Icon` and `IconSVG` to `Button` allows usage as `Button.Icon` and `Button.IconSVG`. Avoid assigning `IconSVG` to `Icon`, as it would require accessing it via `Button.Icon.IconSVG`, which is less convenient.
:::
