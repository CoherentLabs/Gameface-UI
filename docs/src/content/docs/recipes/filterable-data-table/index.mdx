...
The CLI pulls in whatever `FilterableDataTable` depends on. To refresh an existing copy, [update it](/docs/getting-started/updating-components) instead.

## Usage

No props - just render it:

<CodePreviewGFUI envCss={`[class*="dropdown-options"] [class*="content-wrapper"] { max-height: 15rem; }`}>
```tsx
