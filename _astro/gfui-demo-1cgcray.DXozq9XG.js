import { g as getNextElement, u as use, D as setAttribute, t as template, b as createMemo, j as createEffect, k as on, d as batch, o as onMount, c as createComponent, E as createContext, A as delegateEvents, G as useContext, a as createSignal, i as insert, B as runHydrationEvents, r as render } from './web.DoGxwvvO.js';
import { c as createStore, u as unwrap } from './store.BChLrIuc.js';
import { b as baseComponent, m as mappings, u as useNavigation, n as navigationActions, g as gamepad } from './BaseComponent.DmBIQgSj.js';
...
const bag = "/_astro/bag.CwHP8W8x.svg";
...
const boots = "/_astro/boots.DqqelQWn.svg";
...
const breastPlate = "/_astro/breastPlate.BwZT1M7z.svg";
...
const helmet = "/_astro/helmet.Cu_204zx.svg";
...
const pants = "/_astro/pants.VuPxdWXl.svg";
...
const shield = "/_astro/shield.L02FUHzS.svg";
...
const sword = "/_astro/sword.DuW02L9u.svg";
...
const roundStar = "/_astro/round-star.BSX5ziS4.svg";
...
function normalizeIconKey(name, file) {
  const key = name.replace(/[^a-zA-Z0-9]+(.)?/g, (_, chr) => chr ? chr.toUpperCase() : "").replace(/^[A-Z]/, (c) => c.toLowerCase());
  if (key === "") {
    throw new Error(
      `Icon "${file}" has no usable name: it contains no letters or digits. Rename it to something like "myIcon.svg".`
    );
  }
  if (/^[0-9]/.test(key)) {
    throw new Error(
      `Icon "${file}" produces the key "${key}", which starts with a digit — <Icon.${key} /> is not valid syntax. Rename it to start with a letter, e.g. "icon${key}.svg".`
    );
  }
  return key;
}
