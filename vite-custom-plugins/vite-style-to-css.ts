...
import { JSXAttribute, JSXExpressionContainer, JSXOpeningElement, ObjectProperty } from "@babel/types";
 ...
 */
function transformInlinedStyles(code: string, isBuild = false) {
    let cssOutput = "";
...
export default function solidStyleToCssPlugin(): Plugin {
    let isBuild = false;
    ...
    return {
        ...
        transform: {
            ...
            handler(code, id) {
                ...
                const { ast, cssOutput } = transformInlinedStyles(code, isBuild);
        ...
        handleHotUpdate({ file, server, modules }) {
            ...
            const { ast, cssOutput } = transformInlinedStyles(code);
