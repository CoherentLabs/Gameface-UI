...
import { ScrollContentTokenizer } from './slots/tokenizers';
import Content from './slots/Content';
import Bar, { BarComponent } from './slots/Bar';
...
const Scroll = (props) => {
    ...
    const ContentTokens = resolveTokens(ScrollContentTokenizer, () => props.children);

    return (
        <ScrollContext.Provider value={{scrollByClickHandler}}>
            <LayoutBase {...props} refObject={scrollObjectRef}>
                <div ref={containerRef!} class={styles.Scroll}>
                    <div ref={contentRef!} class={styles.Content}>
                        {ContentTokens()[0].data.props.children}
                    </div>
                    {overflow() && (
                        <BarComponent />
                    )}
...
export default Object.assign(Scroll, {Content, Bar});